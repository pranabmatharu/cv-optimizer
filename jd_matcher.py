...
Uses pdfplumber for text extraction, regex for section detection
"""
import re
import pdfplumber
import fitz  # PyMuPDF
...
# ── Section patterns (order matters — more specific first) ──────────────────
SECTION_PATTERNS = {
...
def extract_text_from_pdf(file_bytes: bytes) -> tuple[str, int]:
    """Extract full text from PDF bytes using pdfplumber (better for CVs)."""
    text_pages = []
    page_count = 0
    try:
        import io
...
def detect_section(line: str) -> Optional[str]:
    ...
    # Heuristics: all-caps, or short line, or followed by underline chars
    is_header_like = (
        stripped.isupper()
        or stripped.istitle()
...
def extract_bullets(text: str) -> list[str]:
    """Pull bullet points from a section's text."""
    bullets = []
...
def segment_sections(full_text: str, raw_lines: list[str]) -> dict[str, CVSection]:
    ...
    for i, line in enumerate(raw_lines):
        ...
        if detected:
            ...
            # Lines before any section = contact info
            elif line.strip():
                if "contact" not in sections:
                    sections["contact"] = CVSection(
                        name="contact",
                        raw_title="Contact",
                        content="",
                        line_start=0,
...
def parse_cv(file_bytes: bytes) -> ParsedCV:
    """Main entry point: parse a PDF CV into structured sections."""
...
def extract_contact_info(cv: ParsedCV) -> dict:
    """Try to extract email, phone, LinkedIn from full text."""
    text = cv.full_text
